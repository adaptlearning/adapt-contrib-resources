...
class ResourcesHelpers {
  ...
  resources_get_column_count(resources) {
    // add 1 for the 'All' button column
...
const helpers = Object.getOwnPropertyNames(Object.getPrototypeOf(new ResourcesHelpers()));

helpers.filter(name => name != 'constructor').forEach(value => {
  Handlebars.registerHelper(value, new ResourcesHelpers()[value]);
});

export default ResourcesHelpers;
